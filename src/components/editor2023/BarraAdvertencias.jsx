...
export default function BarraAdvertencias({ errores }) {
  ...
  return (
    <div className="sticky top-0 z-20 bg-amber-50 dark:bg-amber-950/40 border-b border-amber-200 dark:border-amber-900">
      <button
      ...
      {!colapsado && (
        <ul className="px-6 pb-3 space-y-0.5">
          {errores.map((e, i) => (
            <li key={i} className="text-xs text-amber-800 dark:text-amber-200">• {e}</li>
          ))}
        </ul>
      )}
    </div>
