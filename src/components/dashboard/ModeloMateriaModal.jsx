...
import { MODELO_2018, MODELO_2023 } from '../../services/materias'

const MODELOS = [
  {
    id: MODELO_2018,
    nombre: 'Modelo 2018',
    desc: 'Formato tradicional con tablas detalladas. Compatible con planeaciones existentes.',
    badge: null,
  ...
  },
  {
    id: MODELO_2023,
    nombre: 'Modelo 2023',
    desc: 'Formato CONALEP actual. Organiza momentos de inicio, desarrollo y cierre.',
    badge: 'Actual',
          ...
          <div className="grid grid-cols-2 gap-3">
            {MODELOS.map(m => (
              <button
                ...
                className={`selectable-card relative rounded-2xl border-2 p-4 text-left
                  ${modelo === m.id
              ...
              >
                {m.badge && modelo !== m.id && (
                  <span className="absolute right-2 top-2 rounded-full bg-document-100 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-document-700 dark:bg-document-900/40 dark:text-document-300">
                    {m.badge}
                  </span>
                )}
                {modelo === m.id && (
                ...
                <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl
                  ${modelo === m.id ? 'bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-700/60 dark:text-slate-400'}`}>
